...
public class CategoryAccLegs extends AdvCategory {
	...
	public CategoryAccLegs(MercyCustomizationScreen parent) {
        ...
        this.addCombo(new SteinComboButton(2, parent.width / 2 - 124, j-47, 168, 4, "Legs: " + (plr.getAccessory("legs")+1) + "/2", plr.getAccessory("legs"), (val, type) -> {
        	if(type == Type.MODEL) {
	        	plr.updateAccessory("legs", val.getValue());
	        	val.setMessage("Legs: " + (plr.getAccessory("legs")+1) + "/2");
        	}
        	if(type == Type.VARIATION) {
        		plr.updateAccessory("legs_var", val.getVariation());
        	}
        }).setVariant(plr.getAccessory("legs_var")));
